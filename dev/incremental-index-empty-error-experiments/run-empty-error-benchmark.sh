...
baseline_repo="${BASELINE_REPO:-$script_dir}"
candidate_repo="${CANDIDATE_REPO:-$(git -C "$script_dir" rev-parse --show-toplevel)}"
...
baseline_source="$baseline_repo/baseline-source/IncrementalIndex.java"
...
compile_sources()
{
...

  {
    ...
    if [[ -f "$results_dir/DimensionPresenceBenchmark.measured.java" ]]; then
      echo "benchmark-source-sha256-measured: $(shasum -a 256 "$results_dir/DimensionPresenceBenchmark.measured.java" | awk '{print $1}')"
    fi
    echo "benchmark-source-sha256-final: $(shasum -a 256 "$benchmark_source" | awk '{print $1}')"
...
case "$stage" in
  ...
  run)
    ...
    validate_variant baseline
    validate_variant candidate
    for scenario in sixDense sixtyFourDense; do
