...
public class SQLMetadataStorageActionHandlerTest
{
  ...
  public void testEntryAndStatus()
  {
    ...
    JUnit5Assertions.assertEquals(Optional.of(entry), handler.getEntry(entryId));
    JUnit5Assertions.assertEquals(Optional.absent(), handler.getEntry("non_exist_entry"));
    JUnit5Assertions.assertEquals(Optional.absent(), handler.getStatus(entryId));
    JUnit5Assertions.assertEquals(Optional.absent(), handler.getStatus("non_exist_entry"));
    JUnit5Assertions.assertTrue(handler.setStatus(entryId, true, status1));
    ...
    JUnit5Assertions.assertTrue(handler.setStatus(entryId, true, status2));
    ...
    JUnit5Assertions.assertTrue(handler.setStatus(entryId, false, status1));
    ...
    JUnit5Assertions.assertFalse(handler.setStatus(entryId, false, status2));

    JUnit5Assertions.assertEquals(Optional.of(status1), handler.getStatus(entryId));
    JUnit5Assertions.assertEquals(Optional.of(entry), handler.getEntry(entryId));
    JUnit5Assertions.assertEquals(
        ImmutableList.of(),
  ...
  public void testGetRecentStatuses()
  {
    ...
    JUnit5Assertions.assertEquals(7, statuses.size());
    for (TaskInfo status : statuses) {
      JUnit5Assertions.assertEquals(TaskState.RUNNING, status.getStatus().getStatusCode());
    }
  }
  ...
  public void testGetRecentStatuses2()
  {
    ...
    JUnit5Assertions.assertEquals(5, statuses.size());
    for (TaskInfo status : statuses) {
      JUnit5Assertions.assertEquals(TaskState.RUNNING, status.getStatus().getStatusCode());
    }
  }
  ...
  public void testDuplicateInsertThrowsEntryExistsException()
  {
    ...
    JUnit5Assertions.assertEquals(exception.getErrorCode(), "invalidInput");
    JUnit5Assertions.assertEquals(StringUtils.format("Task [%s] already exists", entryId), exception.getMessage());
  }
  ...
  public void testLocks()
  {
    ...
    JUnit5Assertions.assertTrue(handler.getLocks("non_exist_entry").isEmpty());

    JUnit5Assertions.assertTrue(handler.getLocks(entryId).isEmpty());

    final TaskLock lock1 = createRandomLock(entry);
    final TaskLock lock2 = createRandomLock(entry);

    JUnit5Assertions.assertTrue(handler.addLock(entryId, lock1));
    JUnit5Assertions.assertTrue(handler.addLock(entryId, lock2));

    final Map<Long, TaskLock> locks = handler.getLocks(entryId);
    JUnit5Assertions.assertEquals(2, locks.size());
    ...
    JUnit5Assertions.assertEquals(updated.keySet(), locks.keySet());
  }
  ...
  public void testReplaceLock()
  {
    ...
    JUnit5Assertions.assertNotNull(lockId1);

    JUnit5Assertions.assertTrue(handler.replaceLock(entryId, lockId1, lock2));
  }
  ...
  public void testGetLockId()
  {
    ...
    JUnit5Assertions.assertNotNull(handler.getLockId(entryId, lock1));
    JUnit5Assertions.assertNull(handler.getLockId(entryId, lock2));
  }
  ...
  public void testRemoveTasksOlderThan()
  {
    ...
    JUnit5Assertions.assertEquals(Optional.of(entry1), handler.getEntry(entryId1));
    JUnit5Assertions.assertEquals(Optional.of(entry2), handler.getEntry(entryId2));
    JUnit5Assertions.assertEquals(Optional.of(entry3), handler.getEntry(entryId3));
  ...
  public void testMigration()
  {
    ...
    JUnit5Assertions.assertEquals(numActiveTasks + numCompletedTasks, getUnmigratedTaskCount().intValue());

    handler.populateTaskTypeAndGroupId();

    JUnit5Assertions.assertEquals(0, getUnmigratedTaskCount().intValue());
  }
  ...
  public void testGetTaskStatusPlusListInternal()
  {
    ...
    JUnit5Assertions.assertEquals(4, taskMetadataInfos.size());
  ...
  private void verifyTaskInfoToMetadataInfo(TaskInfo taskInfo,
                                            ...
                                            boolean nullNewColumns)
  {
    JUnit5Assertions.assertEquals(taskInfo.getId(), taskMetadataInfo.getTaskIdentifier().getId());
    JUnit5Assertions.assertEquals(taskInfo.getCreatedTime(), taskMetadataInfo.getCreatedTime());
    JUnit5Assertions.assertEquals(taskInfo.getDataSource(), taskMetadataInfo.getDataSource());
    ...
    JUnit5Assertions.assertEquals(task.getId(), taskIdentifier.getId());
    if (nullNewColumns) {
      JUnit5Assertions.assertNull(taskIdentifier.getGroupId());
      JUnit5Assertions.assertNull(taskIdentifier.getType());
    } else {
      JUnit5Assertions.assertEquals(task.getGroupId(), taskIdentifier.getGroupId());
      JUnit5Assertions.assertEquals(task.getType(), taskIdentifier.getType());
    }
  }
