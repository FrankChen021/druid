...
public class RemoteTaskActionClientFactoryTest
{
  ...
  public void test_buildRetryPolicy_withDefaultConfig()
  {
    ...
    JUnit5Assertions.assertEquals(14, retryPolicy.maxAttempts());

    // Default minWait is PT5S (5 seconds)
    JUnit5Assertions.assertEquals(5000, retryPolicy.minWaitMillis());

    // Default maxWait is PT1M (1 minute)
    JUnit5Assertions.assertEquals(60000, retryPolicy.maxWaitMillis());
  }
  ...
  public void test_buildRetryPolicy_withCustomConfig()
  {
    ...
    JUnit5Assertions.assertEquals(6, retryPolicy.maxAttempts());

    // minWait is PT10S (10 seconds)
    JUnit5Assertions.assertEquals(10000, retryPolicy.minWaitMillis());

    // maxWait is PT2M (2 minutes)
    JUnit5Assertions.assertEquals(120000, retryPolicy.maxWaitMillis());
  }
  ...
  public void test_buildRetryPolicy_withZeroRetries()
  {
    ...
    JUnit5Assertions.assertEquals(1, retryPolicy.maxAttempts());

    // minWait is PT1S (1 second)
    JUnit5Assertions.assertEquals(1000, retryPolicy.minWaitMillis());

    // maxWait is PT30S (30 seconds)
    JUnit5Assertions.assertEquals(30000, retryPolicy.maxWaitMillis());
  }
}
