...
public class NoticesQueueTest
{
  ...
  public void testQueue() throws InterruptedException
  {
    ...
    for (int i = 0; i < 3; i++) {
      JUnit5Assertions.assertEquals(0, queue.size());
      queue.add("xyz");
      JUnit5Assertions.assertEquals(1, queue.size());

      queue.add("xyz");
      JUnit5Assertions.assertEquals(1, queue.size());

      queue.add("foo");
      JUnit5Assertions.assertEquals(2, queue.size());

      queue.add("xyz");
      JUnit5Assertions.assertEquals(2, queue.size());

      queue.add("bar");
      JUnit5Assertions.assertEquals(3, queue.size());

      JUnit5Assertions.assertEquals(queue.poll(10), "xyz");
      JUnit5Assertions.assertEquals(queue.poll(10), "foo");
      JUnit5Assertions.assertEquals(queue.poll(10), "bar");
      JUnit5Assertions.assertNull(queue.poll(10));
      JUnit5Assertions.assertEquals(0, queue.size());
    }
  }
  ...
  public void testQueueConcurrent() throws InterruptedException, ExecutionException
  {
    ...
    try {
      ...
      queue.add("xyz");
      JUnit5Assertions.assertEquals(item.get(), "xyz");
    }
    finally {
      exec.shutdownNow();
    }
