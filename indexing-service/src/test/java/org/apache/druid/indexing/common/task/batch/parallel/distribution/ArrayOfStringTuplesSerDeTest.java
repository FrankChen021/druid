...
public class ArrayOfStringTuplesSerDeTest
{
  ...
  public void testSizeOf()
  {
    ...
    JUnit5Assertions.assertEquals(serde.sizeOf(stringTuple), serde.serializeToByteArray(stringTuple).length);
  }

  private void testSerde(StringTuple stringTuple)
  {
    byte[] bytes = serde.serializeToByteArray(stringTuple);
    JUnit5Assertions.assertEquals(serde.sizeOf(stringTuple), bytes.length);

    Memory wrappedMemory = Memory.wrap(bytes);
    JUnit5Assertions.assertEquals(serde.sizeOf(wrappedMemory, 0, 1), bytes.length);

    StringTuple[] deserialized = serde.deserializeFromMemory(wrappedMemory, 1);
    JUnit5Assertions.assertArrayEquals(new StringTuple[]{stringTuple}, deserialized);
  }

  private void testSerde(StringTuple[] inputArray)
  {
    byte[] bytes = serde.serializeToByteArray(inputArray);
    JUnit5Assertions.assertEquals(serde.sizeOf(inputArray), bytes.length);

    Memory wrappedMemory = Memory.wrap(bytes);
    JUnit5Assertions.assertEquals(serde.sizeOf(wrappedMemory, 0, inputArray.length), bytes.length);

    StringTuple[] deserialized = serde.deserializeFromMemory(wrappedMemory, inputArray.length);
    JUnit5Assertions.assertArrayEquals(inputArray, deserialized);
  }
