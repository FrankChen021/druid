...
public class GlobalTaskLockboxTest
{
  ...
  @Test
  @ExpectedToThrow(IllegalStateException.class)
  ...
  public void testLockAfterTaskComplete()
  {
    ...
    JUnit5Assertions.assertTrue(exception.getMessage().contains("Unable to grant lock to inactive Task"));
  }
  ...
  public void testTryMixedLocks()
  {
    ...
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, lowPriorityTask, interval1).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.SHARED, lowPriorityTask, interval2).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.SHARED, lowPriorityTask2, interval2).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, lowPriorityTask, interval3).isOk());

    lockbox.add(highPiorityTask);
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.SHARED, highPiorityTask, interval1).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, highPiorityTask, interval2).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, highPiorityTask, interval3).isOk());

    JUnit5Assertions.assertTrue(lockbox.findLocksForTask(lowPriorityTask).stream().allMatch(TaskLock::isRevoked));
    JUnit5Assertions.assertTrue(lockbox.findLocksForTask(lowPriorityTask2).stream().allMatch(TaskLock::isRevoked));
    ...
    lockbox.add(highPiorityTask);
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, highPiorityTask, interval1).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.SHARED, highPiorityTask, interval2).isOk());
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, highPiorityTask, interval3).isOk());

    lockbox.add(lowPriorityTask);
    JUnit5Assertions.assertFalse(tryTimeChunkLock(TaskLockType.SHARED, lowPriorityTask, interval1).isOk());
    JUnit5Assertions.assertFalse(tryTimeChunkLock(TaskLockType.EXCLUSIVE, lowPriorityTask, interval2).isOk());
    JUnit5Assertions.assertFalse(tryTimeChunkLock(TaskLockType.EXCLUSIVE, lowPriorityTask, interval3).isOk());
  }
  ...
  public void testTryExclusiveLock()
  {
    ...
    lockbox.add(task);
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, task, Intervals.of("2015-01-01/2015-01-03")).isOk());

    // try to take lock for task 2 for overlapping interval
    Task task2 = NoopTask.create();
    lockbox.add(task2);
    JUnit5Assertions.assertFalse(tryTimeChunkLock(TaskLockType.EXCLUSIVE, task2, Intervals.of("2015-01-01/2015-01-02")).isOk());

    // task 1 unlocks the lock
    lockbox.remove(task);

    // Now task2 should be able to get the lock
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, task2, Intervals.of("2015-01-01/2015-01-02")).isOk());
  }
  ...
  {
    JUnit5Assertions.assertFalse(tryTimeChunkLock(TaskLockType.EXCLUSIVE, NoopTask.create(), Intervals.of("2015-01-01/2015-01-02")).isOk());
  }
  ...
  public void testTimeoutForLock() throws InterruptedException
  {
    ...
    lockbox.add(task1);
    lockbox.add(task2);
    JUnit5Assertions.assertTrue(acquireTimeChunkLock(TaskLockType.EXCLUSIVE, task1, Intervals.of("2015-01-01/2015-01-02"), 5000).isOk());
    JUnit5Assertions.assertFalse(acquireTimeChunkLock(TaskLockType.EXCLUSIVE, task2, Intervals.of("2015-01-01/2015-01-15"), 1000).isOk());
  }
  ...
  public void testSyncFromStorage()
  {
    ...
    JUnit5Assertions.assertEquals(originalBox.getAllLocks(), newBox.getAllLocks());
    JUnit5Assertions.assertEquals(originalBox.getActiveTasks(), newBox.getActiveTasks());

    final List<TaskLock> afterLocksInStorage = taskStorage.getActiveTasks().stream()
                                                          .flatMap(task -> taskStorage.getLocks(task.getId()).stream())
                                                          .collect(Collectors.toList());

    JUnit5Assertions.assertEquals(beforeLocksInStorage, afterLocksInStorage);
  }
  ...
  public void testSyncFromStorageWithInvalidPriority()
  {
    ...
    JUnit5Assertions.assertEquals(1, result.getTasksToFail().size());
    JUnit5Assertions.assertTrue(result.getTasksToFail().contains(task));
  }
  ...
  public void testSyncWithUnknownTaskTypesFromModuleNotLoaded()
  {
    ...
    JUnit5Assertions.assertEquals(2, tasksFromLoaded.size());
  }
  ...
  public void testRevokedLockSyncFromStorage()
  {
    ...
    JUnit5Assertions.assertTrue(originalBox.tryLock(task1, new TimeChunkLockRequest(TaskLockType.EXCLUSIVE, task1, Intervals.of("2017/2018"), null)).isOk());

    // task2 revokes task1
    final Task task2 = NoopTask.ofPriority(100);
    taskStorage.insert(task2, TaskStatus.running(task2.getId()));
    originalBox.add(task2);
    JUnit5Assertions.assertTrue(originalBox.tryLock(task2, new TimeChunkLockRequest(TaskLockType.EXCLUSIVE, task2, Intervals.of("2017/2018"), null)).isOk());
    ...
    JUnit5Assertions.assertEquals(1, task1Locks.size());
    JUnit5Assertions.assertTrue(task1Locks.get(0).isRevoked());

    final List<TaskLock> task2Locks = beforeLocksInStorage.get(task1.getId());
    JUnit5Assertions.assertEquals(1, task2Locks.size());
    JUnit5Assertions.assertTrue(task2Locks.get(0).isRevoked());
  ...
  public void testDoInCriticalSectionWithSharedLock() throws Exception
  {
    ...
    final Task task = NoopTask.create();
    lockbox.add(task);
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.SHARED, task, interval).isOk());
  ...
  public void testDoInCriticalSectionWithExclusiveLock() throws Exception
  {
    ...
    JUnit5Assertions.assertNotNull(lock);
  ...
  public void testDoInCriticalSectionWithRevokedLock() throws Exception
  {
    ...
    JUnit5Assertions.assertNotNull(lowPriorityLock);
    JUnit5Assertions.assertTrue(tryTimeChunkLock(TaskLockType.EXCLUSIVE, highPriorityTask, interval).isOk());
    JUnit5Assertions.assertTrue(Iterables.getOnlyElement(lockbox.findLocksForTask(lowPriorityTask)).isRevoked());
  ...
  public void testAcquireLockAfterRevoked() throws InterruptedException
  {
    ...
    JUnit5Assertions.assertFalse(lockResult.isOk());
    JUnit5Assertions.assertTrue(lockResult.isRevoked());
    JUnit5Assertions.assertTrue(Iterables.getOnlyElement(lockbox.findLocksForTask(lowPriorityTask)).isRevoked());
  }
  ...
  public void testUnlock()
  {
    ...
    for (int i = 0; i < 4; i++) {
      JUnit5Assertions.assertTrue(taskStorage.getLocks(lowPriorityTasks.get(i).getId()).stream().allMatch(TaskLock::isRevoked));
      JUnit5Assertions.assertFalse(taskStorage.getLocks(highPriorityTasks.get(i).getId()).stream().allMatch(TaskLock::isRevoked));
    }

    for (int i = 4; i < 8; i++) {
      JUnit5Assertions.assertFalse(taskStorage.getLocks(lowPriorityTasks.get(i).getId()).stream().allMatch(TaskLock::isRevoked));
    }
  ...
  public void testFindLockPosseAfterRevokeWithDifferentLockIntervals()
  {
    ...
    JUnit5Assertions.assertTrue(highLockPosse.isPresent());
    JUnit5Assertions.assertTrue(highLockPosse.get().containsTask(highPriorityTask));
    JUnit5Assertions.assertFalse(highLockPosse.get().getTaskLock().isRevoked());
    ...
    JUnit5Assertions.assertTrue(lowLockPosse.isPresent());
    JUnit5Assertions.assertTrue(lowLockPosse.get().containsTask(lowPriorityTask));
    JUnit5Assertions.assertTrue(lowLockPosse.get().getTaskLock().isRevoked());
  }
  ...
  public void testSegmentLock() throws InterruptedException
  {
    ...
    JUnit5Assertions.assertTrue(lockResult.isOk());
    JUnit5Assertions.assertNull(lockResult.getNewSegmentId());
    JUnit5Assertions.assertTrue(lockResult.getTaskLock() instanceof SegmentLock);
    final SegmentLock segmentLock = (SegmentLock) lockResult.getTaskLock();
    JUnit5Assertions.assertEquals(TaskLockType.EXCLUSIVE, segmentLock.getType());
    JUnit5Assertions.assertEquals(task.getGroupId(), segmentLock.getGroupId());
    JUnit5Assertions.assertEquals(task.getDataSource(), segmentLock.getDataSource());
    JUnit5Assertions.assertEquals(Intervals.of("2015-01-01/2015-01-02"), segmentLock.getInterval());
    JUnit5Assertions.assertEquals(segmentLock.getVersion(), "v1");
    JUnit5Assertions.assertEquals(3, segmentLock.getPartitionId());
    JUnit5Assertions.assertEquals(task.getPriority(), segmentLock.getPriority().intValue());
    JUnit5Assertions.assertFalse(segmentLock.isRevoked());
  }
  ...
  public void testSegmentAndTimeChunkLockForSameIntervalWithDifferentPriority()
  {
    ...
    JUnit5Assertions.assertFalse(resultOfTask1.isOk());
    JUnit5Assertions.assertTrue(resultOfTask1.isRevoked());
  }
  ...
  public void testRequestForNewSegmentWithSegmentLock()
  {
    ...
    JUnit5Assertions.assertEquals(5, locks.size());
    int expectedPartitionId = 0;
    for (TaskLock lock : locks) {
      JUnit5Assertions.assertTrue(lock instanceof SegmentLock);
      final SegmentLock segmentLock = (SegmentLock) lock;
      JUnit5Assertions.assertEquals(expectedPartitionId++, segmentLock.getPartitionId());
  ...
  )
  {
    ...
    JUnit5Assertions.assertNotNull(result.getTaskLock());
    JUnit5Assertions.assertTrue(result.getTaskLock() instanceof SegmentLock);
    JUnit5Assertions.assertNotNull(result.getNewSegmentId());
    ...
    JUnit5Assertions.assertEquals(lockRequest.getType(), segmentLock.getType());
    JUnit5Assertions.assertEquals(lockRequest.getGroupId(), segmentLock.getGroupId());
    JUnit5Assertions.assertEquals(lockRequest.getDataSource(), segmentLock.getDataSource());
    JUnit5Assertions.assertEquals(lockRequest.getInterval(), segmentLock.getInterval());
    JUnit5Assertions.assertEquals(lockRequest.getPartialShardSpec().getShardSpecClass(), segmentId.getShardSpec().getClass());
    JUnit5Assertions.assertEquals(lockRequest.getPriority(), segmentLock.getPriority().intValue());
  }
  ...
  public void testLockPosseEquals()
  {
    ...
    JUnit5Assertions.assertNotEquals(taskLockPosse1, null);
    JUnit5Assertions.assertNotEquals(null, taskLockPosse1);
    JUnit5Assertions.assertNotEquals(taskLockPosse1, taskLockPosse2);
    JUnit5Assertions.assertEquals(taskLockPosse1, taskLockPosse3);
  }
  ...
  public void testGetTimeChunkAndSegmentLockForSameGroup()
  {
    ...
    JUnit5Assertions.assertEquals(2, posses.size());

    JUnit5Assertions.assertEquals(LockGranularity.TIME_CHUNK, posses.get(0).getTaskLock().getGranularity());
    final TimeChunkLock timeChunkLock = (TimeChunkLock) posses.get(0).getTaskLock();
    JUnit5Assertions.assertEquals(timeChunkLock.getDataSource(), "none");
    JUnit5Assertions.assertEquals(timeChunkLock.getGroupId(), "groupId");
    JUnit5Assertions.assertEquals(Intervals.of("2017/2018"), timeChunkLock.getInterval());

    JUnit5Assertions.assertEquals(LockGranularity.SEGMENT, posses.get(1).getTaskLock().getGranularity());
    final SegmentLock segmentLock = (SegmentLock) posses.get(1).getTaskLock();
    JUnit5Assertions.assertEquals(segmentLock.getDataSource(), "none");
    JUnit5Assertions.assertEquals(segmentLock.getGroupId(), "groupId");
    JUnit5Assertions.assertEquals(Intervals.of("2017/2018"), segmentLock.getInterval());
    JUnit5Assertions.assertEquals(0, segmentLock.getPartitionId());
  }
  ...
  public void testGetLockedIntervalsForHigherPriorityExclusiveLock()
  {
    ...
    JUnit5Assertions.assertTrue(conflictingIntervals.isEmpty());
  }
  ...
  public void testGetLockedIntervalsForLowerPriorityExclusiveLock()
  {
    ...
    Map<String, List<Interval>> conflictingIntervals =
        lockbox.getLockedIntervals(ImmutableList.of(requestForExclusiveLowerPriorityLock));
    JUnit5Assertions.assertEquals(1, conflictingIntervals.size());
    JUnit5Assertions.assertEquals(
  ...
  public void testGetActiveLocks()
  {
    ...
    JUnit5Assertions.assertEquals(1, activeLocks.size());
    JUnit5Assertions.assertEquals(expectedLocks, new HashSet<>(activeLocks.get("none")));
  }
  ...
  public void testFailedToReacquireTaskLock()
  {
    ...
    JUnit5Assertions.assertEquals(3, taskStorage.getActiveTasks().size());
    ...
    JUnit5Assertions.assertEquals(ImmutableSet.of(taskWithFailingLockAcquisition0, taskWithFailingLockAcquisition1),
  ...
  private class TaskLockboxValidator
  {
    ...
    public TaskLock expectLockCreated(TaskLockType type, Task task, Interval interval)
    {
      ...
      JUnit5Assertions.assertNotNull(lock);
      JUnit5Assertions.assertFalse(lock.isRevoked());
      return lock;
    }
    ...
    {
      final TaskLock lock = tryTaskLock(type, task, interval);
      JUnit5Assertions.assertNull(lock);
    }
    ...
    public void expectRevokedLocks(TaskLock... locks)
    {
      ...
      JUnit5Assertions.assertEquals(allLocks.size() - activeLocks.size(), locks.length);
      for (TaskLock lock : locks) {
        JUnit5Assertions.assertTrue(allLocks.contains(lock.revokedCopy()));
        JUnit5Assertions.assertFalse(activeLocks.contains(lock));
      }
    }
    ...
    public void expectActiveLocks(TaskLock... locks)
    {
      final Set<TaskLock> allLocks = getAllLocks();
      final Set<TaskLock> activeLocks = getAllActiveLocks();
      JUnit5Assertions.assertEquals(activeLocks.size(), locks.length);
      for (TaskLock lock : locks) {
        JUnit5Assertions.assertTrue(allLocks.contains(lock));
        JUnit5Assertions.assertTrue(activeLocks.contains(lock));
      }
    }
