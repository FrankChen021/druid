...
public class HeapMemoryTaskStorageTest
{
  ...
  public void testRemoveTasksOlderThan()
  {
    ...
    JUnit5Assertions.assertNotNull(storage.getTaskInfo(task1.getId()));
    JUnit5Assertions.assertNotNull(storage.getTaskInfo(task2.getId()));

    storage.removeTasksOlderThan(DateTimes.of("3000").getMillis());
    JUnit5Assertions.assertNull(storage.getTaskInfo(task1.getId()));
    JUnit5Assertions.assertNotNull(storage.getTaskInfo(task2.getId()));
  }
  ...
  public void testGetTaskInfos()
  {
    ...
    JUnit5Assertions.assertEquals(1, taskInfosActive.size());
    JUnit5Assertions.assertEquals(task2.getId(), taskInfosActive.get(0).getTask().getId());
    ...
    JUnit5Assertions.assertEquals(1, taskInfosComplete.size());
    JUnit5Assertions.assertEquals(task1.getId(), taskInfosComplete.get(0).getTask().getId());
  }
}
