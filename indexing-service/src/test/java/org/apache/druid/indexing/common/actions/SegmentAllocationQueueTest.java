...
public class SegmentAllocationQueueTest
{
  ...
  public void testConflictingPendingSegment()
  {
    ...
    JUnit5Assertions.assertNotNull(getSegmentId(hourSegmentFuture));
    JUnit5Assertions.assertNull(getSegmentId(halfHourSegmentFuture));
  }
  ...
  public void testFullAllocationQueue()
  {
    ...
    Throwable t = JUnit5Assertions.assertThrows(ISE.class, () -> getSegmentId(future));
    JUnit5Assertions.assertEquals(t.getMessage(), "Segment allocation queue is full. Check the metric `task/action/batch/runTime` "
  ...
  public void testMaxBatchSize()
  {
    ...
    JUnit5Assertions.assertEquals(1, allocationQueue.size());
    SegmentAllocateRequest request =
        allocateRequest().forTask(createTask(TestDataSource.WIKI, "group_1")).build();
    allocationQueue.add(request);
    JUnit5Assertions.assertEquals(2, allocationQueue.size());
  }
  ...
  public void testMultipleRequestsForSameSegment()
  {
    ...
    for (Future<SegmentIdWithShardSpec> future : segmentFutures) {
      JUnit5Assertions.assertEquals(getSegmentId(future), segmentId1);
    }
  ...
  public void testRequestsFailOnLeaderChange()
  {
    ...
    for (Future<SegmentIdWithShardSpec> future : segmentFutures) {
      Throwable t = JUnit5Assertions.assertThrows(ISE.class, () -> getSegmentId(future));
      JUnit5Assertions.assertEquals(t.getMessage(), "Not leader anymore");
    }
  }
  ...
  {
    JUnit5Assertions.assertEquals(0, allocationQueue.size());
    ...
    JUnit5Assertions.assertEquals(expectedCount, allocationQueue.size());
    ...
    JUnit5Assertions.assertNotNull(getSegmentId(futureA));
    JUnit5Assertions.assertNotNull(getSegmentId(futureB));
  }
