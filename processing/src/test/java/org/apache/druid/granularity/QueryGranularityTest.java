...
public class QueryGranularityTest extends InitializedNullHandlingTest
{
  ...
  public void testIterableNone()
  {
    ...
    while (iterator.hasNext()) {
      JupiterAssertions.assertEquals(count, iterator.next().getStartMillis());
      count++;
    }
  }
  ...
  public void testSerializePeriod() throws Exception
  {
    ...
    JupiterAssertions.assertEquals(new PeriodGranularity(new Period("P1D"), null, null), gran);

    // Nonstandard period
    json = "{ \"type\": \"period\", \"period\": \"P2D\" }";
    gran = mapper.readValue(json, Granularity.class);
    JupiterAssertions.assertEquals(new PeriodGranularity(new Period("P2D"), null, null), gran);

    // Set timeZone, origin
    json = "{ \"type\": \"period\", \"period\": \"P1D\","
           + "\"timeZone\": \"America/Los_Angeles\", \"origin\": \"1970-01-01T00:00:00Z\"}";
    gran = mapper.readValue(json, Granularity.class);
    JupiterAssertions.assertEquals(new PeriodGranularity(
        new Period("P1D"),
        DateTimes.EPOCH,
    ...
    JupiterAssertions.assertEquals(expected, mapper.readValue(jsonOut, Granularity.class));

    String illegalJson = "{ \"type\": \"period\", \"period\": \"P0D\" }";
    try {
      mapper.readValue(illegalJson, Granularity.class);
      JupiterAssertions.fail();
    }
  ...
  public void testSerializeDuration() throws Exception
  {
    ...
    Granularity gran = mapper.readValue(json, Granularity.class);
    JupiterAssertions.assertEquals(new DurationGranularity(3600000, null), gran);

    json = "{ \"type\": \"duration\", \"duration\": \"5\", \"origin\": \"2012-09-01T00:00:00.002Z\" }";
    gran = mapper.readValue(json, Granularity.class);
    JupiterAssertions.assertEquals(new DurationGranularity(5, 2), gran);

    DurationGranularity expected = new DurationGranularity(5, 2);
    JupiterAssertions.assertEquals(expected, mapper.readValue(mapper.writeValueAsString(expected), Granularity.class));
  ...
  {
    JupiterAssertions.assertNull(Granularity.mergeGranularities(null));
    JupiterAssertions.assertNull(Granularity.mergeGranularities(ImmutableList.of()));
    JupiterAssertions.assertNull(Granularity.mergeGranularities(Lists.newArrayList(null, Granularities.DAY)));
    JupiterAssertions.assertNull(Granularity.mergeGranularities(Lists.newArrayList(Granularities.DAY, null)));
  ...
  {
    JupiterAssertions.assertEquals(expected.size(), Iterables.size(actual));
    Iterator<DateTime> actualIter = actual.iterator();
    Iterator<DateTime> expectedIter = expected.iterator();

    while (actualIter.hasNext() && expectedIter.hasNext()) {
      JupiterAssertions.assertEquals(expectedIter.next(), actualIter.next());
    }
    JupiterAssertions.assertFalse("actualIter not exhausted!?", actualIter.hasNext());
    JupiterAssertions.assertFalse("expectedIter not exhausted!?", expectedIter.hasNext());
  }
  ...
  private void assertSameInterval(List<DateTime> expected, Iterable<Interval> actual)
  {
    ...
    while (actualIter.hasNext() && expectedIter.hasNext()) {
      JupiterAssertions.assertEquals(expectedIter.next(), actualIter.next().getStart());
    }
    JupiterAssertions.assertFalse("actualIter not exhausted!?", actualIter.hasNext());
    JupiterAssertions.assertFalse("expectedIter not exhausted!?", expectedIter.hasNext());
  }
  ...
  public void testToVirtualColumn()
  {
    ...
    JupiterAssertions.assertEquals("timestamp_floor(__time,'PT1H',null,'UTC')", column.getExpression());
    ...
    JupiterAssertions.assertEquals("timestamp_floor(__time,'PT1H',null,'America/Los_Angeles')", column.getExpression());
    column = Granularities.toVirtualColumn(duration, Granularities.GRANULARITY_VIRTUAL_COLUMN_NAME);
    JupiterAssertions.assertEquals(ColumnHolder.TIME_COLUMN_NAME, column.getExpression());
    ...
    JupiterAssertions.assertNull(column);
    ...
    JupiterAssertions.assertEquals("timestamp_floor(__time,'PT1M',null,'UTC')", column.getExpression());
    column = Granularities.toVirtualColumn(Granularities.FIFTEEN_MINUTE, Granularities.GRANULARITY_VIRTUAL_COLUMN_NAME);
    JupiterAssertions.assertEquals("timestamp_floor(__time,'PT15M',null,'UTC')", column.getExpression());
  }
  ...
  public void testFromVirtualColumn()
  {
    ...
    JupiterAssertions.assertEquals(Granularities.HOUR, Granularities.fromVirtualColumn(hourly));
    JupiterAssertions.assertEquals(Granularities.DAY, Granularities.fromVirtualColumn(day));
    JupiterAssertions.assertEquals(Granularities.HOUR, Granularities.fromVirtualColumn(hourlyNonstandardTime));
    JupiterAssertions.assertEquals(
    ...
    JupiterAssertions.assertNull(Granularities.fromVirtualColumn(ceilHour));
    JupiterAssertions.assertEquals(Granularities.MINUTE, Granularities.fromVirtualColumn(floorWithExpression));
    final DateTime origin = DateTimes.of("2012-01-02T05:00:00.000-08:00");
    final DateTimeZone tz = DateTimes.inferTzFromString("America/Los_Angeles");
    ...
    JupiterAssertions.assertEquals(minuteWithTz, Granularities.fromVirtualColumn(floorWithTimezone));
    JupiterAssertions.assertEquals(minuteWithOrigin, Granularities.fromVirtualColumn(floorWithOriginTimezone));
  }
  ...
  public void testFromVirtualColumnExtra()
  {
    ...
    JupiterAssertions.assertEquals(Granularities.ALL, Granularities.fromVirtualColumn(literalField));
  }
