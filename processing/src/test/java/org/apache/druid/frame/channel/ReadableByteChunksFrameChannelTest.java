...
public class ReadableByteChunksFrameChannelTest
{
  ...
  public static class NonParameterizedTests extends InitializedNullHandlingTest
  {
    ...
    public void testZeroBytes()
    {
      ...
      JupiterAssertions.assertTrue(channel.canRead());
      JupiterAssertions.assertFalse(channel.isFinished());
      JupiterAssertions.assertTrue(channel.isErrorOrFinished());
    ...
    public void testEmptyFrameFile() throws IOException
    {
      ...
      channel.doneWriting();
      JupiterAssertions.assertEquals(file.length(), channel.getBytesAdded());
      ...
      JupiterAssertions.assertTrue(channel.isFinished());
      channel.close();
    }
    ...
    public void testTruncatedFrameFile() throws IOException
    {
      ...
      channel.doneWriting();
      JupiterAssertions.assertEquals(truncatedFile.length, channel.getBytesAdded());
      ...
      expectedException.expect(IllegalStateException.class);
      expectedException.expectMessageStartsWith("Incomplete or missing frame at end of stream");
      channel.readFrame();
    }
    ...
    public void testSetError() throws IOException
    {
      ...
      JupiterAssertions.assertEquals(chunk1.length, channel.getBytesAdded());
  ...
  public static class ParameterizedWithTestIndexTests extends InitializedNullHandlingTest
  {
    ...
    public void testWriteFullyThenRead() throws IOException
    {
      ...
      long totalSize = 0;
      JupiterAssertions.assertEquals(0, channel.getBytesBuffered());
      ...
      try (final Chunker chunker = new Chunker(new FileInputStream(file), chunkSize)) {
        ...
        while ((chunk = chunker.nextChunk()) != null) {
          ...
          JupiterAssertions.assertEquals(channel.getBytesAdded(), totalSize);

          // Minimally-sized channel means backpressure is exerted as soon as a single frame is available.
          JupiterAssertions.assertEquals(channel.canRead(), backpressureFuture != null);

          if (backpressureFuture != null) {
            if (firstBackpressureFuture == null) {
              firstBackpressureFuture = backpressureFuture;
            } else {
              JupiterAssertions.assertSame(firstBackpressureFuture, backpressureFuture);
            }
          }
        }

        // Backpressure should be exerted right now, since this is a minimal channel with at least one full frame in it.
        JupiterAssertions.assertNotNull(firstBackpressureFuture);
        JupiterAssertions.assertFalse(firstBackpressureFuture.isDone());
      ...
      try (final Chunker chunker = new Chunker(new FileInputStream(file), chunkSize)) {
        ...
        while ((chunk = chunker.nextChunk()) != null) {
          ...
          if (iteration % 3 == 0) {
            ...
            JupiterAssertions.assertTrue(backpressureFuture == null || backpressureFuture.isDone());
          ...
          JupiterAssertions.assertEquals(totalSize, channel.getBytesAdded());

          // Minimally-sized channel means backpressure is exerted as soon as a single frame is available.
          JupiterAssertions.assertEquals(channel.canRead(), addVal != null);

          if (addVal != null) {
            if (backpressureFuture == null) {
              backpressureFuture = addVal;
            } else {
              JupiterAssertions.assertSame(backpressureFuture, addVal);
            }
          }
        }
    ...
    public void testReadRac() throws IOException
    {
      ...
      while (channel.canRead()) {
        ...
        JupiterAssertions.assertNotNull("Frame should be retrievable from RAC", frame);
      ...
      JupiterAssertions.assertEquals(originalRows, readRows);
    }
