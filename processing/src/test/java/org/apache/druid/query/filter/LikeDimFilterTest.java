...
public class LikeDimFilterTest extends InitializedNullHandlingTest
{
  ...
  public void testSerde() throws IOException
  {
    ...
    JupiterAssertions.assertEquals(filter, filter2);
  }
  ...
  public void testGetCacheKey()
  {
    ...
    JupiterAssertions.assertArrayEquals(filter.getCacheKey(), filter2.getCacheKey());
    JupiterAssertions.assertFalse(Arrays.equals(filter.getCacheKey(), filter3.getCacheKey()));
  }
  ...
  public void testEqualsAndHashCode()
  {
    ...
    final DimFilter filter3 = new LikeDimFilter("foo", "bar%", null, new SubstringDimExtractionFn(1, 2));
    JupiterAssertions.assertEquals(filter, filter2);
    JupiterAssertions.assertNotEquals(filter, filter3);
    JupiterAssertions.assertEquals(filter.hashCode(), filter2.hashCode());
    JupiterAssertions.assertNotEquals(filter.hashCode(), filter3.hashCode());
  }
  ...
  {
    final DimFilter filter = new LikeDimFilter("foo", "bar%", "@", new SubstringDimExtractionFn(1, 2));
    JupiterAssertions.assertEquals(filter.getRequiredColumns(), Sets.newHashSet("foo"));
  }
  ...
  public void testPrefixMatchUsesRangeIndex()
  {
    ...
    JupiterAssertions.assertSame("likeFilter returns the intended bitmapColumnIndex", bitmapColumnIndex, retVal);
  }
  ...
  public void testGetDimensionRangeSet_midPatternWildcard_returnsNull()
  {
    ...
    JupiterAssertions.assertNull(filter.getDimensionRangeSet("foo"));
  }
  ...
  {
    final LikeDimFilter filter = new LikeDimFilter("foo", "bar%", null, null);
    JupiterAssertions.assertNull(filter.getDimensionRangeSet("other"));
  }
  ...
  {
    JupiterAssertions.assertEquals(Range.closedOpen("foo", "fop"), LikeDimFilter.prefixRange("foo"));
  }

  @Test
  public void testPrefixRange_uppercaseCarryStaysWithinAscii()
  {
    JupiterAssertions.assertEquals(Range.closedOpen("foZ", "fo["), LikeDimFilter.prefixRange("foZ"));
  }
  ...
  {
    JupiterAssertions.assertEquals(Range.atLeast("￿￿"), LikeDimFilter.prefixRange("￿￿"));
  }

  @Test
  public void testPrefixRange_empty_throws()
  {
    JupiterAssertions.assertThrows(DruidException.class, () -> LikeDimFilter.prefixRange(""));
  }
  ...
  public void testPrefixRange_enclosesAllPrefixedStrings()
  {
    ...
    JupiterAssertions.assertTrue(range.contains("foo"));
    JupiterAssertions.assertTrue(range.contains("foo0"));
    JupiterAssertions.assertTrue(range.contains("foobar"));
    JupiterAssertions.assertTrue(range.contains("foozzz"));
    JupiterAssertions.assertFalse(range.contains("fo"));
    JupiterAssertions.assertFalse(range.contains("fop"));
    JupiterAssertions.assertFalse(range.contains("fox"));
  }

  @Test
  public void testLexicographicSuccessor_basic()
  {
    JupiterAssertions.assertEquals("fop", LikeDimFilter.lexicographicSuccessor("foo"));
  }

  @Test
  public void testLexicographicSuccessor_empty_returnsNullChar()
  {
    JupiterAssertions.assertEquals("\u0000", LikeDimFilter.lexicographicSuccessor(""));
  }

  @Test
  public void testLexicographicSuccessor_singleMaxValue_returnsNull()
  {
    JupiterAssertions.assertNull(LikeDimFilter.lexicographicSuccessor("￿"));
  }
  ...
  private void assertCompilation(String pattern, String expected)
  {
    ...
    JupiterAssertions.assertEquals(pattern + " => " + expected, matcher.describeCompilation());
  }

  private void assertMatch(String pattern, String value, DruidPredicateMatch expected)
  {
    LikeDimFilter.LikeMatcher matcher = LikeDimFilter.LikeMatcher.from(pattern, '\\');
    JupiterAssertions.assertEquals(matcher + " matches " + value, expected, matcher.matches(value));
  }
}
