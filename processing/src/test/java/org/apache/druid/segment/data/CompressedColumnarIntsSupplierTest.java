...
public class CompressedColumnarIntsSupplierTest extends CompressionStrategyTest
{
  ...
  private void makeWithSerde(final int chunkSize) throws IOException
  {
    ...
    JupiterAssertions.assertEquals(theSupplier.getSerializedSize(), bytes.length);
  ...
  public void testSanity()
  {
    ...
    JupiterAssertions.assertEquals(4, supplier.getBaseIntBuffers().size());
    ...
    setupSimple(32);
    JupiterAssertions.assertEquals(1, supplier.getBaseIntBuffers().size());
    assertIndexMatchesVals();
  }
  ...
  public void testLargeChunks() throws Exception
  {
    ...
    JupiterAssertions.assertEquals(10, supplier.getBaseIntBuffers().size());
    assertIndexMatchesVals();

    setupLargeChunks(maxChunkSize, 10 * maxChunkSize + 1);
    JupiterAssertions.assertEquals(11, supplier.getBaseIntBuffers().size());
    assertIndexMatchesVals();
  ...
  {
    JupiterAssertions.assertEquals(vals.length, columnarInts.size());

    // sequential access
    int[] indices = new int[vals.length];
    for (int i = 0, size = columnarInts.size(); i < size; ++i) {
      JupiterAssertions.assertEquals(vals[i], columnarInts.get(i), 0.0);
      indices[i] = i;
    }

    int[] offsetValues = new int[columnarInts.size() + 1];
    columnarInts.get(offsetValues, 1, 0, columnarInts.size());
    JupiterAssertions.assertEquals(0, offsetValues[0]);
    JupiterAssertions.assertArrayEquals(vals, Arrays.copyOfRange(offsetValues, 1, offsetValues.length));
    ...
    for (int i = 0; i < limit; ++i) {
      int k = indices[i];
      JupiterAssertions.assertEquals(vals[k], columnarInts.get(k), 0.0);
    }
  }
}
