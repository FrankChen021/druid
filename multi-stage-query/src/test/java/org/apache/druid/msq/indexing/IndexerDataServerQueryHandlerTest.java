...
public class IndexerDataServerQueryHandlerTest
{
  ...
  public void testFetchRowsFromServer() throws ExecutionException, InterruptedException
  {
    ...
    JUnitAssertions.assertTrue(dataServerQueryResult.getHandedOffSegments().getDescriptors().isEmpty());
    ...
    events.forEach(
        event -> {
          JUnitAssertions.assertArrayEquals(event.toArray(), yielder.get());
          yielder.next(null);
        }
    );
  }
  ...
  public void testOneSegmentRelocated() throws ExecutionException, InterruptedException
  {
    ...
    ((List<List<Object>>) scanResultValue1.getEvents()).forEach(
        event -> {
          JUnitAssertions.assertArrayEquals(event.toArray(), yielder1.get());
          yielder1.next(null);
        }
    );
    ...
    ((List<List<Object>>) scanResultValue2.getEvents()).forEach(
        event -> {
          JUnitAssertions.assertArrayEquals(event.toArray(), yielder2.get());
          yielder2.next(null);
        }
    );
  }
  ...
  public void testHandoff() throws ExecutionException, InterruptedException
  {
    ...
    JUnitAssertions.assertEquals(ImmutableList.of(SEGMENT_1, SEGMENT_2), dataServerQueryResult.getHandedOffSegments().getDescriptors());
    JUnitAssertions.assertTrue(dataServerQueryResult.getResultsYielders().isEmpty());
  }
  ...
  public void testServerNotFoundWithoutHandoffShouldThrowException()
  {
    ...
    JUnitAssertions.assertThrows(DruidException.class, () ->
