...
{
  private static final TaskStorage TASK_STORAGE = mock(TaskStorage.class);
  private static final TaskMaster TASK_MASTER = mock(TaskMaster.class);
  private static final TaskRunner TASK_RUNNER = mock(TaskRunner.class);
  private static final StorageConnector STORAGE_CONNECTOR = mock(StorageConnector.class);
  private static final TaskRunnerWorkItem TASK_RUNNER_WORK_ITEM = mock(TaskRunnerWorkItem.class);
  ...
  public void setUp()
  {
    reset(TASK_STORAGE, TASK_RUNNER, TASK_RUNNER_WORK_ITEM, STORAGE_CONNECTOR, TASK_MASTER);
  ...
  {
    when(TASK_STORAGE.getCompletedTasksInfo(any(), isNull())).thenReturn(List.of());
    when(STORAGE_CONNECTOR.listDir(anyString())).thenReturn(
        ImmutableList.of(DurableStorageUtils.getControllerDirectory(TASK_ID), STRAY_DIR).iterator()
    );
    when(TASK_RUNNER_WORK_ITEM.getTaskId()).thenReturn(TASK_ID);
    doReturn(ImmutableList.of(TASK_RUNNER_WORK_ITEM)).when(TASK_RUNNER).getRunningTasks();
    when(TASK_MASTER.getTaskRunner()).thenReturn(Optional.of(TASK_RUNNER));
    final ArgumentCaptor<Set<String>> capturedArguments = ArgumentCaptor.captor();

    durableStorageCleaner.run();

    verify(STORAGE_CONNECTOR).deleteFiles(capturedArguments.capture());
    JUnitAssertions.assertEquals(Sets.newHashSet(STRAY_DIR), capturedArguments.getValue());
  }
  ...
  public void testRunClearsStaleOrNotFoundTask() throws Exception
  {
    ...
    when(TASK_RUNNER_WORK_ITEM.getTaskId()).thenReturn(TASK_ID);
    doReturn(ImmutableList.of()).when(TASK_RUNNER).getRunningTasks();
    when(TASK_MASTER.getTaskRunner()).thenReturn(Optional.of(TASK_RUNNER));
    final ArgumentCaptor<Set<String>> capturedArguments = ArgumentCaptor.captor();
    ...
    verify(STORAGE_CONNECTOR).deleteFiles(capturedArguments.capture());
    JUnitAssertions.assertEquals(Set.of(DurableStorageUtils.getControllerDirectory(TASK_ID)), capturedArguments.getValue());
  }
  ...
  public void testRunExcludesQueryDirectory() throws Exception
  {
    ...
    when(TASK_STORAGE.getCompletedTasksInfo(any(), isNull())).thenReturn(
    ...
    verify(STORAGE_CONNECTOR).deleteFiles(capturedArguments.capture());
    JUnitAssertions.assertEquals(Sets.newHashSet(STRAY_DIR, intermediateFilesPath), capturedArguments.getValue());
  }
  ...
  public void testGetSchedule()
  {
    ...
    JUnitAssertions.assertEquals(cleanerConfig.delaySeconds * 1000, schedule.getPeriodMillis());
    JUnitAssertions.assertEquals(cleanerConfig.delaySeconds * 1000, schedule.getInitialDelayMillis());
  }
}
