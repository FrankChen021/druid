...
public class WorkerSketchFetcherTest
{
  ...
  public void test_submitFetcherTask_parallelFetch() throws InterruptedException
  {
    ...
    JUnitAssertions.assertTrue(latch.await(5, TimeUnit.SECONDS));
  ...
  public void test_sequentialMerge_nonCompleteInformation()
  {
    ...
    target = spy(new WorkerSketchFetcher(workerClient, workerManager, true, SketchEncoding.OCTET_STREAM));
    JUnitAssertions.assertThrows(ISE.class, () -> target.sequentialTimeChunkMerging(
        (ignore) -> {},
        completeKeyStatisticsInformation,
        stageDefinition.getId(),
        ImmutableSet.of(""),
  ...
  public void test_inMemoryRetryEnabled_retryInvoked() throws InterruptedException
  {
    ...
    JUnitAssertions.assertTrue(latch.await(500, TimeUnit.SECONDS));
    JUnitAssertions.assertTrue(retryLatch.await(500, TimeUnit.SECONDS));
  }
  ...
  public void test_SequentialRetryEnabled_retryInvoked() throws InterruptedException
  {
    ...
    JUnitAssertions.assertTrue(latch.await(5, TimeUnit.SECONDS));
    JUnitAssertions.assertTrue(retryLatch.await(5, TimeUnit.SECONDS));
  }
  ...
  public void test_InMemoryRetryDisabled_multipleFailures() throws InterruptedException
  {
    ...
    try {
      ...
      JUnitAssertions.assertTrue(e.getMessage().contains("Task fetch failed"));
    }

    while (!target.executorService.isShutdown()) {
      Thread.sleep(100);
    }
    JUnitAssertions.assertTrue((target.getError().getMessage().contains("Task fetch failed")));
  ...
  public void test_SequentialRetryDisabled_singleFailure() throws InterruptedException
  {
    ...
    JUnitAssertions.assertTrue(target.getError().getMessage().contains(TASK_1));
