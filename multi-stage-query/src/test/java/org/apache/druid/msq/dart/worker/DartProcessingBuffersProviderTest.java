...
public class DartProcessingBuffersProviderTest
{
  ...
  public void test_acquire_poolSizeZero()
  {
    ...
    JUnitAssertions.assertNotNull(result);
    JUnitAssertions.assertEquals(ProcessingBuffersSet.EMPTY, result.get());
  ...
  public void test_acquire_singleSliceUsesFullChunk()
  {
    ...
    try {
      ...
      try {
        ...
        JUnitAssertions.assertEquals(1, buffers.getBouncer().getMaxCount());

        final ResourceHolder<ByteBuffer> sliceHolder = buffers.getBufferPool().take();
        JUnitAssertions.assertEquals(BUFFER_SIZE, sliceHolder.get().capacity());
        sliceHolder.close();
      }
      finally {
        holder.close();
  ...
  public void test_acquire_processingThreadsSlices()
  {
    ...
    try {
      ...
      try {
        final ProcessingBuffers buffers = holder.get();
        JUnitAssertions.assertEquals(PROCESSING_THREADS, buffers.getBouncer().getMaxCount());

        final List<ResourceHolder<ByteBuffer>> sliceHolders = new ArrayList<>();
        try {
          for (int i = 0; i < PROCESSING_THREADS; i++) {
            final ResourceHolder<ByteBuffer> sliceHolder = buffers.getBufferPool().take();
            JUnitAssertions.assertEquals(BUFFER_SIZE / PROCESSING_THREADS, sliceHolder.get().capacity());
  ...
  public void test_acquire_resliceAfterRelease()
  {
    ...
    try {
      ...
      JUnitAssertions.assertEquals(2, holder1.get().getBouncer().getMaxCount());
      JUnitAssertions.assertEquals(BUFFER_SIZE / 2, holder1.get().getBufferPool().take().get().capacity());
      holder1.close();
      ...
      JUnitAssertions.assertEquals(4, holder2.get().getBouncer().getMaxCount());
      JUnitAssertions.assertEquals(BUFFER_SIZE / 4, holder2.get().getBufferPool().take().get().capacity());
      holder2.close();
    }
    finally {
      result.close();
  ...
  public void test_acquire_poolSizeTwo()
  {
    ...
    JUnitAssertions.assertNotNull(buffersSet);
    ...
    for (int i = 0; i < poolSize; i++) {
      ...
      try {
        ...
        for (int j = 0; j < PROCESSING_THREADS; j++) {
          final ResourceHolder<ByteBuffer> sliceHolder = buffers.getBufferPool().take();
          JUnitAssertions.assertEquals(expectedSliceSize, sliceHolder.get().capacity());
  ...
  public void test_acquire_timeout()
  {
    ...
    MSQException exception = JUnitAssertions.assertThrows(
        MSQException.class,
        () -> provider.acquire(1, TIMEOUT_MILLIS)
    );

    JUnitAssertions.assertTrue(exception.getFault() instanceof CanceledFault);
    JUnitAssertions.assertEquals("Canceled", exception.getFault().getErrorCode());
  }
}
