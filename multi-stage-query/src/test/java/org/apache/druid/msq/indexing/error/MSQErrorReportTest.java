...
public class MSQErrorReportTest
{
  ...
  {
    JUnitAssertions.assertEquals(UnknownFault.forException(null), MSQErrorReport.getFaultFromException(null));

    MSQException msqException = new MSQException(null, UnknownFault.forMessage(ERROR_MESSAGE));
    JUnitAssertions.assertEquals(msqException.getFault(), MSQErrorReport.getFaultFromException(msqException));
    ...
    JUnitAssertions.assertEquals(new TooManyBucketsFault(10), MSQErrorReport.getFaultFromException(tooManyBucketsException));

    FrameRowTooLargeException tooLargeException = new FrameRowTooLargeException(10);
    JUnitAssertions.assertEquals(new RowTooLargeFault(10), MSQErrorReport.getFaultFromException(tooLargeException));

    UnexpectedMultiValueDimensionException mvException = new UnexpectedMultiValueDimensionException(ERROR_MESSAGE);
    JUnitAssertions.assertEquals(DruidExceptionFault.CODE, MSQErrorReport.getFaultFromException(mvException).getErrorCode());
