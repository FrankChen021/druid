...
public record SystemTablePushdownFilter(String key, @Nullable String value)
{
  ...
  {
    final List<DimFilter> extracted = new ArrayList<>(extract(query.getFilter(), pushdownFilters));
    if (query instanceof WindowOperatorQuery && !pushdownFilters.isEmpty()) {
      final Map<String, String> columnMappings = columnMappings(pushdownFilters);
  ...
  /** Extracts and rewrites the storage-safe conjuncts from a Druid filter. */
  public static List<DimFilter> extract(
  ...
  )
  {
    ...
    extractConjuncts(filter, columnMappings(pushdownFilters), extracted);
    return Collections.unmodifiableList(extracted);
  }

  private static Map<String, String> columnMappings(final List<SystemTablePushdownFilter> pushdownFilters)
  {
    return pushdownFilters.stream().collect(
        Collectors.toMap(
  ...
  public static boolean isStringValuesFilter(final DimFilter filter)
  {
  ...
  public static String getStringValuesColumn(final DimFilter filter)
  {
    return switch (filter) {
      ...
      case OrDimFilter or -> getStringValuesColumn(or.getFields().get(0));
      case null, default -> throw new IllegalArgumentException("Filter does not contain a finite set of strings");
    };
  }
