...
public class ServerPropertiesTableDataProvider implements SystemTableDataProvider
{
  ...
  )
  {
    ...
    for (final DimFilter filter : filters) {
      if (!SystemTablePushdownFilter.isStringValuesFilter(filter)) {
        continue;
      }
      ...
      if ("server".equals(column) && !values.contains(server)
          || "service_name".equals(column) && !values.contains(selfNode.getServiceName())) {
