...
:::info
 ...
 `druid.sql.planner.enableSysQueriesTable=true` on Broker processes.
:::

The queries table provides information about query executions. With native system-table execution enabled, the table
includes initial user queries and their distributed executions from Brokers, Historicals, Overlords, Indexers, and
Peons. The bindable execution path continues to show Dart queries only.
...
|info|`COMPLEX<json>`|Query information including `sqlQueryId`, `sql`, `identity`, `startTime`, and other engine-specific details|
|initial_query_id|VARCHAR|ID of the initial user query. Druid uses the ID supplied by the user, or the ID generated by the query framework when the user does not supply one. All distributed executions for a query share this ID. This column is `NULL` on the bindable execution path.|
|is_initial_query|BIGINT|Boolean represented as a long where 1 indicates the initial user query and 0 indicates a distributed execution. This column is `NULL` on the bindable execution path.|
|server|VARCHAR|Host and port of the server reporting this execution. This column is `NULL` on the bindable execution path.|
|server_type|VARCHAR|Comma-separated node roles of the server reporting this execution. This column is `NULL` on the bindable execution path.|

The native execution path requires `READ` access to the state resource. Enable it with the
`useNativeQueryForSystemTables` query context parameter. For example, to retrieve all executions for one initial query:

```sql
SELECT *
FROM sys.queries
WHERE initial_query_id = 'my-query-id'
```
